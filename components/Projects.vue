...
const projectsList: Array<projectDTO> = [{
    title: "Vulcan Novels",
    started: new Date(),
    description: "Lorem ipsum dolor sit, amet consectetur adipisicing elit. Ipsa ullam modi obcaecati, quis culpa a excepturi harum saepe officiis dolore eos? Ab, numquam? Numquam quos consequuntur animi tenetur, deserunt ratione!",
    tecnologies: [{
        title: "Vue",
        icon: "uil:vuejs"
    }, {
    ...
    github: "https://github.com/devdouglasgfs/",
...
<template>
    <section ref="section" class="flex w-full gap-8 section-with-observer portfolio-section relative"
        :class="{ 'translate-y-0': sectionVisible || triggered }">
        <div class="size-max">
            <Icon class="title text-primary-500" name="mdi:developer-board" />
        </div>
        <svg class="absolute -z-10 isolate top-6 left-0 max-w-full overflow-x-hidden" width="650"
        ...
        <div class="flex flex-col gap-6 lg:gap-8 w-full">
            ...
            <ul class="flex relative flex-col gap-6 self-stretch">
                <li v-for="project in projectsList" class="w-full">
                    <Project :title="project.title" :started="project.started" :description="project.description"
                        :finished="project.finished" :github="project.github" :image="project.image"
                        :tecnologies="project.tecnologies" :deploy-url="project.deployUrl" />
                </li>
            </ul>
        </div>
    </section>
